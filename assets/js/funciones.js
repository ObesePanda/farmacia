      ...
      .append(
        `
                <div style="white-space: normal;">
                    ...
                    <small>💲 Precio: $${parseFloat(item.precio).toFixed(
                      2
                    )}</small>
                </div>
            `
      ...
      success: function (res) {
        if (res.status) {
          ...
          Swal.fire({
            ...
            html:
              `<p>${res.mensaje}</p><ul style="text-align:left;">` +
...
function obtenerDetallesVentaConLote() {
  ...
  $("#detalle_venta tr").each(function () {
    ...
    const descuento =
      parseFloat($(this).find("td").eq(3).find("input").val()) || 0;
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      response.forEach((lote) => {
        const vencido =
          lote.vencimiento && new Date(lote.vencimiento) < new Date()
            ? " (VENCIDO)"
            : "";
        options += `<option value="${lote.id}" 
                    ${
                      lote.vencimiento &&
                      new Date(lote.vencimiento) < new Date()
                        ? "disabled"
                        : ""
                    }>
                    ${lote.numero_lote} - ${lote.cantidad} unds - vence ${
          lote.vencimiento || "N/A"
        }${vencido}
                </option>`;
      });
...
function renderDetalle() {
  ...
  detalle.forEach((item, index) => {
    ...
    tbody.innerHTML += `
      <tr>
        <td>${item.codigo} <small class="text-muted">(ID: ${
      item.id
    })</small></td>
        <td>${item.cantidad}</td>
  ...
  fetch(`get_lotes.php?id=${idProducto}`)
    ...
    .then((data) => {
      ...
      if (data.length > 0) {
        data.forEach((lote) => {
          const vencido =
            lote.fecha_vencimiento && lote.fecha_vencimiento < hoy;

          html += `
                        <tr class="${vencido ? "table-danger" : ""}">
                            <td>${lote.lote}</td>
                            <td class="${
                              vencido ? "text-danger fw-bold" : "text-success"
                            }">
                                ${
                                ...
                                ${
                                  vencido
                                    ? '<i class="bi bi-exclamation-triangle-fill ms-1" title="Lote vencido"></i>'
                                    : ""
                                }
                            </td>
                            <td>${lote.cantidad}</td>
                            ...
                            <td>
                                <form action="eliminar_lote.php" method="post" class="d-inline confirmar">
